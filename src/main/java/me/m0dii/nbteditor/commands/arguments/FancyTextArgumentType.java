...
import me.m0dii.nbteditor.fancytext.StyleOption;
...
public class FancyTextArgumentType implements ArgumentType<Text> {
    ...
    public static ArgumentType<UnparsedText> fancyText() {
        ...
        return new ArgumentType<>() {
            @Override
            public UnparsedText parse(StringReader reader) {
    ...
    public static String stringifyFancyText(Text text, Style base, boolean printErrors) {
        ...
        if (Boolean.TRUE.equals(output.getValue()) && printErrors) {
            MiscUtil.client.player.sendMessage(TextInst.translatable("nbteditor.fancy_text_arg_type.stringify_unsupported"), false);
        }
    ...
    public interface UnparsedText {
        Text parse(Style base) throws CommandSyntaxException;
    }
