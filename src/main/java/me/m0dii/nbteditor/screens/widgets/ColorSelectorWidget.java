...
import me.m0dii.nbteditor.misc.Shaders;
...
public class ColorSelectorWidget extends GroupWidget {
    ...
    private static final Identifier HUES = IdentifierInst.of("m0-dev-tools", "textures/hues.png");
    private final int x;
    private final int y;
    ...
    public ColorSelectorWidget(int x, int y, int areaSize, int color, Consumer<Integer> onColor) {
        ...
        addWidget(new MVSliderWidget(x, y + areaSize + 4, areaSize, 20, hueValue / 359.0,
            ...
            protected boolean renderSlider(MatrixStack matrices, int mouseX, int mouseY, float delta) {
                DrawableHelper.drawTexture(matrices, HUES, x + 4, y, 0, 0, width - 8, 20, width - 8, 20);
                return true;
            }
        ...
        field = new TextFieldWidget(MiscUtil.client.textRenderer, x + areaSize + 4, y + areaSize + 4, areaSize / 2, 20, TextInst.of(""));
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        DrawableHelper.fill(matrices, x + areaSize + 4, y, x + areaSize + 4 + areaSize / 2, y + areaSize, color | 0xFF000000);
    }
    ...
    private class ColorArea implements MVDrawable, MVElement {
        @Override
        public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
            MiscUtil.fillShader(matrices, Shaders.POSITION_HSV, vertex -> vertex.light(hueValue), x, y, areaSize, areaSize);
        }
