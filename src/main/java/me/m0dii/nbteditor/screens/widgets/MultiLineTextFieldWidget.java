...
public class MultiLineTextFieldWidget implements MVDrawable, MVElement, Tickable, Selectable {
    ...
    private static final TextRenderer textRenderer = MiscUtil.client.textRenderer;
    ...
    protected MultiLineTextFieldWidget(int x,
                                       int y,
                                       int width,
                                       int height,
                                       String text,
        ...
        this.undo.addFirst(Map.entry(text, text.length()));
        this.undoPos = 0;
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        DrawableHelper.enableScissor(matrices, x, y, width, height);
        ...
        for (Text line : renderedLines) {
            DrawableHelper.drawText(matrices, textRenderer, line, x + textRenderer.fontHeight, yOffset + textRenderer.fontHeight, -1, shadow);
        ...
        if (isMultiFocused() && cursorBlinkTracker / 6 % 2 == 0) {
            ...
            DrawableHelper.fill(matrices, cursor.x, cursor.y, cursor.x + 1, cursor.y + textRenderer.fontHeight, cursorColor);
        }
    ...
    protected void renderHighlight(MatrixStack matrices, int start, int end, int color) {
        ...
        if (startPos.y == endPos.y) {
            DrawableHelper.fill(matrices, startPos.x, startPos.y, endPos.x, endPos.y + textRenderer.fontHeight, color);
        } else {
            int line = 0;
            int lineY;
            while ((lineY = startPos.y + line * (int) (textRenderer.fontHeight * 1.5)) < endPos.y) {
                ...
                DrawableHelper.fill(matrices, lineStart.x, lineStart.y, x + width - textRenderer.fontHeight, lineStart.y + textRenderer.fontHeight, color);
                line++;
            }
            DrawableHelper.fill(matrices, x + textRenderer.fontHeight, lineY, endPos.x, endPos.y + textRenderer.fontHeight, color);
        }
    }
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        if (Screen.isCopy(keyCode)) {
            MiscUtil.client.keyboard.setClipboard(onCopy(getSelectedText(), getSelStart(), getSelEnd() - getSelStart()));
            return true;
        }
        if (Screen.isPaste(keyCode)) {
            this.write(pasteFilter(onPaste(MiscUtil.client.keyboard.getClipboard(), getSelStart(), getSelEnd() - getSelStart())));
            cursorX = -1;
            return true;
        }
    ...
    private class FindAndReplaceWidget extends TranslatedGroupWidget {
        ...
        public FindAndReplaceWidget() {
            super(MiscUtil.client.getWindow().getScaledWidth() / 2 - 100,
                    MiscUtil.client.getWindow().getScaledHeight() / 2 - 30, 200);
        ...
        public void renderPre(MatrixStack matrices, int mouseX, int mouseY, float delta) {
            DrawableHelper.fill(matrices, -16, -16, 216, 76, 0xC8101010);
        }
