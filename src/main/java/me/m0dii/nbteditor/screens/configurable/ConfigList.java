...
import me.m0dii.nbteditor.screens.widgets.InputOverlay;
import me.m0dii.nbteditor.screens.widgets.StringInput;
...
public class ConfigList extends ConfigGroupingVertical<Integer, ConfigList> {
    ...
    public Map<Integer, ConfigPath> getConfigurables() {
        return paths.entrySet().stream()
                ...
                .collect(Collectors.toMap(Map.Entry::getKey, entry
                        -> ((ConfigListEntry) entry.getValue()).value, (a, b) -> a, LinkedHashMap::new));
    }
        ...
        if (isValueValid()) {
            ...
            for (ConfigPath path : paths.values()) {
                ...
                if (path instanceof ConfigListEntry) {
                    ...
                    DrawableHelper.drawTextWithShadow(matrices, MiscUtil.client.textRenderer, TextInst.of("⋮"), 0,
                            -MiscUtil.client.textRenderer.fontHeight / 2, -1);
                    matrices.pop();
                }
    ...
    private static class ConfigListEntry implements ConfigPath {
        ...
        private static final int LIST_CONTEXT_MENU_HEIGHT = (MiscUtil.client.textRenderer.fontHeight + 2) * 3 + 2;
        ...
        public void setIndex(int index) {
            ...
            if (indexed) {
                ...
                this.indexTextOffset = named ? 0 : MiscUtil.client.textRenderer.getWidth(this.indexText);
                if (named) {
        ...
        public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
            if (named) {
                ...
                DrawableHelper.drawTextWithShadow(matrices, MiscUtil.client.textRenderer, indexText, 0, (getSpacingHeight() - MiscUtil.client.textRenderer.fontHeight) / 2, -1);
                matrices.push();
        ...
        public void renderContextMenu(MatrixStack matrices, int mouseX, int mouseY, float delta) {
            ...
            DrawableHelper.fill(matrices, contextMenuX - 1, contextMenuY - 1, contextMenuX + 51, contextMenuY + LIST_CONTEXT_MENU_HEIGHT + 1, -1);
            DrawableHelper.fill(matrices, contextMenuX, contextMenuY, contextMenuX + 50, contextMenuY + LIST_CONTEXT_MENU_HEIGHT, 0xFF000000);
            ...
            for (ListContextMenuAction action : ListContextMenuAction.values()) {
                int color = -1;
                if (xHover && mouseY >= y && mouseY <= y + MiscUtil.client.textRenderer.fontHeight) {
                    color = 0xFF257789;
                ...
                DrawableHelper.drawCenteredTextWithShadow(matrices, MiscUtil.client.textRenderer, msg, contextMenuX + 25, y + 2, color);
                y += MiscUtil.client.textRenderer.fontHeight + 2;
            }
            ...
            if (contextMenuOpen) {
                if (mouseX >= contextMenuX && mouseX <= contextMenuX + 50 && mouseY >= contextMenuY && mouseY <= contextMenuY + LIST_CONTEXT_MENU_HEIGHT) {
                    if (mouseX > contextMenuX && mouseX < contextMenuX + 50) {
                        int y = contextMenuY;
                        for (ListContextMenuAction action : ListContextMenuAction.values()) {
                            if (mouseY >= y && mouseY <= y + MiscUtil.client.textRenderer.fontHeight) {
                                switch (action) {
                                    case MOVE -> {
                                        InputOverlay.show(
                                                ...
                                                StringInput.builder()
                                                        ...
                                                        .withValidator(
                                                                MiscUtil.intPredicate(() -> 1, () -> parent.paths.size() - 1, false))
                                                        .build(),
                                                str -> {
                                        ...
                                        if (Screen.hasShiftDown()) {
                                            InputOverlay.show(
                                                    ...
                                                    StringInput.builder()
                                                            ...
                                                            .withValidator(
                                                                    MiscUtil.intPredicate(1, Integer.MAX_VALUE, false))
                                                            .build(),
