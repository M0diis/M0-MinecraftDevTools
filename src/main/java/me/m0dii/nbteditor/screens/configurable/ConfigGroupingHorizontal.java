...
public abstract class ConfigGroupingHorizontal<K, T extends ConfigGroupingHorizontal<K, T>> extends ConfigGrouping<K, T> {
    ...
    protected int getNameWidth() {
        return name == null ? 0 : MiscUtil.client.textRenderer.getWidth(name) + PADDING;
    }
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        if (fullName != null) {
            DrawableHelper.drawTextWithShadow(matrices, MiscUtil.client.textRenderer, fullName, PADDING * 2, 0, 0xFFFFFFFF);
