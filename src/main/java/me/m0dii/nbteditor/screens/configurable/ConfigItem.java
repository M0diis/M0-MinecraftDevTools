...
public class ConfigItem<V extends ConfigValue<?, V>> implements ConfigPath {
    ...
    public ConfigItem(Text name, V value) {
        this.name = name;
        this.value = value;
        this.valueOffsetX = MiscUtil.client.textRenderer.getWidth(name) + PADDING;
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        DrawableHelper.drawTextWithShadow(matrices, MiscUtil.client.textRenderer, name, 0, (getSpacingHeight() - MiscUtil.client.textRenderer.fontHeight) / 2, 0xFFFFFFFF);
