...
public class ConfigValueDropdown<T> extends MVButtonWidget implements ConfigValue<T, ConfigValueDropdown<T>> {
    ...
    private ConfigValueDropdown(T value, T defaultValue, List<T> allValues, List<T> importantValues) {
        super(0, 0, getMaxWidth(allValues) + MiscUtil.client.textRenderer.fontHeight * 2, 20, TextInst.of(value.toString()),
    ...
    private static int getMaxWidth(List<?> allValues) {
        return allValues.stream().map(Object::toString).mapToInt(MiscUtil.client.textRenderer::getWidth).max().orElse(0);
    }
    ...
    public void renderButton(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        matrices.push();
        matrices.translate(0.0, 0.0, (MiscUtil.client.getWindow().getScaledHeight() - translation[1]) / 20);

        super.renderButton(matrices, mouseX, mouseY, delta);
        if (open) {
            DrawableHelper.fill(matrices, this.x, this.height, this.x + this.width, allValues.size() * this.height, 0xFF000000);
            ...
            for (T option : allValues) {
                ...
                DrawableHelper.drawCenteredTextWithShadow(matrices, MiscUtil.client.textRenderer, TextInst.of(option.toString()),
                        this.x + this.width / 2, y + (this.height - MiscUtil.client.textRenderer.fontHeight) / 2, color);
