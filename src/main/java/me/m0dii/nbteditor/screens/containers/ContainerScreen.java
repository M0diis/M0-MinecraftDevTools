...
import me.m0dii.nbteditor.nbtreferences.itemreferences.ContainerItemReference;
...
public class ContainerScreen<L extends LocalNBT> extends ClientHandledScreen {
    ...
    public static <L extends LocalNBT> void show(NBTReference<L> ref) {
        ...
        M0DevToolsClient.CURSOR_MANAGER.showBranch(new ContainerScreen<>(ref));
    }
    ...
    public LockedSlotsInfo getLockedSlotsInfo() {
        LockedSlotsInfo info = (ref instanceof ItemReference itemRef && itemRef.isLocked()
                ? LockedSlotsInfo.ITEMS_LOCKED
                : LockedSlotsInfo.NONE).copy();
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        if (MiscUtil.client.options.inventoryKey.matchesKey(keyCode, scanCode)) {
            ref.showParent();
            return true;
        }
