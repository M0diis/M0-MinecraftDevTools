...
public class MVSliderWidget extends MVButtonWidget {
    ...
    private static final Identifier HANDLE = IdentifierInst.of("m0-dev-tools", "textures/slider_handle.png");
    private static final Identifier HANDLE_HIGHLIGHTED = IdentifierInst.of("m0-dev-tools", "textures/slider_handle_highlighted.png");
    ...
    public MVSliderWidget(int x, int y, int width, int height, double value, Supplier<Text> msg, Consumer<Double> onValue, MVTooltip tooltip) {
        super(x, y, width, height, msg.get(), btn -> {
        }, tooltip);
        this.value = value;
        this.msg = msg;
    ...
    public void renderButton(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        if (renderSlider(matrices, mouseX, mouseY, delta)) {
            DrawableHelper.drawTexture(matrices, this.hovered || this.isFocused() ? HANDLE_HIGHLIGHTED : HANDLE,
                    x + (int) (value * (width - 8)), y, 0, 0, 8, 20, 8, 20);
            DrawableHelper.drawCenteredTextWithShadow(matrices, MiscUtil.client.textRenderer, getMessage(),
                    x + width / 2, y + height / 2 - MiscUtil.client.textRenderer.fontHeight / 2, -1);
        } else {
            DrawContext context = DrawableHelper.getDrawContext(matrices);
