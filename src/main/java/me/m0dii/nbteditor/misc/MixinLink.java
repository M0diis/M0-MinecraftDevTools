...
import me.m0dii.nbteditor.async.ItemSize;
import me.m0dii.nbteditor.containers.ContainerIO;
import me.m0dii.nbteditor.mixin.ChatScreenAccessor;
import me.m0dii.nbteditor.mixin.HandledScreenAccessor;
...
public class MixinLink {
    ...
    public static int[] getTooltipSize(List<TooltipComponent> tooltip) {
        ...
        for (TooltipComponent line : tooltip) {
            width = Math.max(width, line.getWidth(MiscUtil.client.textRenderer));
            height += line.getHeight(MiscUtil.client.textRenderer);
        }
    ...
    public static void renderTooltipFromComponents(MatrixStack matrices, int x, int y, int width, int height, int screenWidth, int screenHeight) {
        ...
        MiscUtil.mapMatrices(matrices, x, y, width, height, newX, newY, newWidth, newHeight);
    }
    ...
    public static void renderChatLimitWarning(ChatScreen source, MatrixStack matrices) {
        ...
        if (chatField.getText().length() > 256) {
            ...
            DrawableHelper.drawCenteredTextWithShadow(matrices, textRenderer, TextInst.translatable("nbteditor.chat_length_warning_1"), source.width - 102, source.height - 40 + textRenderer.fontHeight / 2, 0xFFAA5500);
            DrawableHelper.drawCenteredTextWithShadow(matrices, textRenderer, TextInst.translatable("nbteditor.chat_length_warning_2"), source.width - 102, source.height - 28 + textRenderer.fontHeight / 2, 0xFFAA5500);
        }
    }
    ...
    public static void onMouseClick(HandledScreen<?> source, Slot slot, int slotId, int button, SlotActionType actionType, CallbackInfo info) {
        ...
        if (!creativeInv && !M0DevToolsClient.SERVER_CONN.isScreenEditable()) {
            return;
        }
        ...
        if (actionType == SlotActionType.PICKUP && slot != null &&
                (slot.inventory == MiscUtil.client.player.getInventory() || !creativeInv) &&
            ...
            if (cursor.getItem() == Items.ENCHANTED_BOOK || item.getItem() == Items.ENCHANTED_BOOK) {
                ...
                M0DevToolsClient.CURSOR_MANAGER.setCursor(ItemStack.EMPTY);
    ...
    public static void keyPressed(HandledScreen<?> source, int keyCode, int scanCode, int modifiers, CallbackInfoReturnable<Boolean> info) {
        ...
        if (hoveredSlot != null &&
                ((creativeInv && hoveredSlot.inventory == MiscUtil.client.player.getInventory()) ||
                        (!creativeInv && M0DevToolsClient.SERVER_CONN.isScreenEditable())) &&
    ...
    public static void modifyTooltip(ItemStack source, List<Text> tooltip) {
        ...
        if (source.contains(MVComponentType.HIDE_TOOLTIP)) {
            return;
        }
        ...
        if (!ConfigScreen.isKeybindsHidden()) {
            ...
            if (creativeInv || (!(MiscUtil.client.currentScreen instanceof CreativeInventoryScreen) &&
                    M0DevToolsClient.SERVER_CONN.isScreenEditable())) {
