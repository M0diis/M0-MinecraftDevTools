...
public record FancyTextStyleOptionNode(StyleOption option, String value,
                                       List<FancyTextNode> contents) implements FancyTextNode {
    ...
    public Style modifyStyle(Style style) {
        return switch (option) {
            ...
            case SHOW_ITEM -> {
                ItemStack item;
                try {
                    item = MiscUtil.client.player.getInventory().getStack(Integer.parseInt(value));
            ...
            case SHOW_ENTITY -> {
                ...
                try {
                    ...
                    entity = StreamSupport.stream(MiscUtil.client.world.getEntities().spliterator(), false)
                            .filter(testEntity -> testEntity.getUuid().equals(uuidObj)).findFirst()
                            .orElseThrow(IllegalArgumentException::new);
                } catch (IllegalArgumentException e) {
                    if (MiscUtil.client.targetedEntity != null) {
                        entity = MiscUtil.client.targetedEntity;
                    } else {
                        entity = MiscUtil.client.player;
                    }
                }
