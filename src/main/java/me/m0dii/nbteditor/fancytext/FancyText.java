    ...
    private static EditableText gen(List<FancyTextNode> nodes, Style base) {
        ...
        for (FancyTextNode node : nodes) {
            switch (node) {
                ...
                default -> style = node.modifyStyle(style);
            }
        }
        return numberOfTextNodes == 1 ? (EditableText) output.getSiblings().getFirst() : output;
    }
