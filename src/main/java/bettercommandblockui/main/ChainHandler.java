...
public class ChainHandler {
    ...
    public ChainHandler(BetterCommandBlockScreen screen, BlockPos pos) {
        ...
        this.currentPos = pos;//BlockPos.ofFloored(executor.getPos());
    ...
    public void scanChain() {
        ...
        if ((up != null) && up.get(CommandBlock.FACING) == Direction.DOWN) {
            prior.add(new Pair<>(up, Direction.UP));
        }
        if ((down != null) && down.get(CommandBlock.FACING) == Direction.UP) {
            prior.add(new Pair<>(down, Direction.DOWN));
        }
        if ((north != null) && north.get(CommandBlock.FACING) == Direction.SOUTH) {
            prior.add(new Pair<>(north, Direction.NORTH));
        }
        if ((south != null) && south.get(CommandBlock.FACING) == Direction.NORTH) {
            prior.add(new Pair<>(south, Direction.SOUTH));
        }
        if ((east != null) && east.get(CommandBlock.FACING) == Direction.WEST) {
            prior.add(new Pair<>(east, Direction.EAST));
        }
        if ((west != null) && west.get(CommandBlock.FACING) == Direction.EAST) {
            prior.add(new Pair<>(west, Direction.WEST));
        }

        if (!prior.isEmpty()) {
            prior.sort((o1, o2) -> {
                if (o1.getLeft().get(CommandBlock.FACING) == currentDir) {
                    return -1;
                }
                if (o2.getLeft().get(CommandBlock.FACING) == currentDir) {
                    return 1;
                }
                return 0;
            });
