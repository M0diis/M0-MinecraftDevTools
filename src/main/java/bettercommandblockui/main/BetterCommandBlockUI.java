...
public class BetterCommandBlockUI implements ClientModInitializer {
    ...
    public static void setConfig(String key, String value) {
        ...
        if (key.equals(VAR_SCROLL_X)) {
            SCROLL_STEP_X = CONFIG.getOrDefault(key, 4);
        }
        if (key.equals(VAR_SCROLL_Y)) {
            SCROLL_STEP_Y = CONFIG.getOrDefault(key, 2);
        }
        if (key.equals(VAR_INDENTATION)) {
            INDENTATION_FACTOR = CONFIG.getOrDefault(key, 2);
        }
        if (key.equals(VAR_WRAPAROUND)) {
            WRAPAROUND_WIDTH = CONFIG.getOrDefault(key, 250);
        }
        if (key.equals(VAR_FORMAT_STRINGS)) {
            FORMAT_STRINGS = CONFIG.getOrDefault(key, true);
        }
        if (key.equals(VAR_IGNORE_ENTER)) {
            IGNORE_ENTER = CONFIG.getOrDefault(key, false);
        }
        if (key.equals(VAR_AUTOSAVE)) {
            AUTOSAVE = CONFIG.getOrDefault(key, false);
        }
        if (key.equals(VAR_NEWLINE_PRE_OPEN_BRACKET)) {
            NEWLINE_PRE_OPEN_BRACKET = CONFIG.getOrDefault(key, true);
        }
        if (key.equals(VAR_NEWLINE_POST_OPEN_BRACKET)) {
            NEWLINE_POST_OPEN_BRACKET = CONFIG.getOrDefault(key, true);
        }
        if (key.equals(VAR_NEWLINE_PRE_CLOSE_BRACKET)) {
            NEWLINE_PRE_CLOSE_BRACKET = CONFIG.getOrDefault(key, true);
        }
        if (key.equals(VAR_NEWLINE_POST_CLOSE_BRACKET)) {
            NEWLINE_POST_CLOSE_BRACKET = CONFIG.getOrDefault(key, false);
        }
        //if(key.equals(VAR_NEWLINE_POST_LAST_CLOSE_BRACKET)) NEWLINE_POST_LAST_CLOSE_BRACKET = CONFIG.getOrDefault(key, false);
        if (key.equals(VAR_NEWLINE_POST_COMMA)) {
            NEWLINE_POST_COMMA = CONFIG.getOrDefault(key, true);
        }
        if (key.equals(VAR_AVOID_DOUBLE_NEWLINE)) {
            AVOID_DOUBLE_NEWLINE = CONFIG.getOrDefault(key, true);
        }
        //if(key.equals(VAR_BRACKET_AUTOCOMPLETE)) BRACKET_AUTOCOMPLETE = CONFIG.getOrDefault(key, false);
        if (key.equals(VAR_TRACK_OUTPUT_DEFAULT_USED)) {
            TRACK_OUTPUT_DEFAULT_USED = CONFIG.getOrDefault(key, false);
        }
        if (key.equals(VAR_TRACK_OUTPUT_DEFAULT_VALUE)) {
            TRACK_OUTPUT_DEFAULT_VALUE = CONFIG.getOrDefault(key, true);
        }
        if (key.equals(VAR_SHOW_OUTPUT_DEFAULT)) {
            SHOW_OUTPUT_DEFAULT = CONFIG.getOrDefault(key, false);
        }
    }
